...
const clientID = `YOUR CLIENT INFO HERE (SEE README.MD)`;

const clientSecret = `YOUR CLIENT INFO HERE (SEE README.MD)`;

const nextEvent = getNextEvent("./skill/lambda/custom/schedule.txt");

if (nextEvent.daysTillEvent === 1 || true) {
  // adjust as needed

  notify(
    nextEvent.mediaEventName,
    nextEvent.mediaEventTime,
    nextEvent.mediaEventProvider
  ); // Multicast
}
...
async function notify(mediaEventName, mediaEventTime, mediaEventProvider) {
  ...
  console.log(
    `Sent notification for ${mediaEventName} at ${mediaEventTime} on ${mediaEventProvider}`
  );
...
function sendEvent(token, mediaEventName, mediaEventTime, mediaEventProvider) {
  return new Promise((resolve) => {
    ...
    const ProactivePostData = JSON.stringify(
      getMediaEvent(mediaEventName, mediaEventTime, mediaEventProvider)
    );
...
function getNextEvent(dataFile) {
  ...
  const scheduleFile = fs.readFileSync(dataFile, function (err, data) {
    if (err) {
      console.log("error reading file: " + dataFile + "\n" + err);
    }
  });
